...
export function useAdminData() {
  ...
  const metadataProgressTimer = useRef<number | null>(null);
  ...
  function startMetadataProgressTimer(initialProgress: number) {
  ...
  async function extractMetadata(version: MetadataExtractionVersion, batchSize: number) {
    if (!selectedSiteId) return;
  ...
  async function updateCluster(clusterId: string, status?: "approved" | "rejected", label?: string) {
